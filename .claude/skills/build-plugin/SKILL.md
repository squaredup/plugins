...
2. **If a join key is missing, add it to `objectMapping.properties` now.** Adding it here is free if that step's objects haven't been imported yet; if they have (a dependent-step build where Checkpoint B already ran for a lower level), every existing object is now stale — apply the [re-indexing rule](#re-indexing-rule).
...
⚠️ **A definition change leaves imported objects stale.** The objects now in the graph are **frozen at import time**: any later edit to `indexDefinitions/*.json` or an import stream leaves every existing object **stale** until the datasource is re-imported — a property you map now is absent on every already-imported object.
