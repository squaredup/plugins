...
// dataStreams/scripts/emailRouting.js
...
// user-selected breakdown (dimensions.action vs dimensions.status vs ...).
...
const groups =
    (data &&
        ...
        data.data.viewer.zones[0].emailRoutingAdaptiveGroups) ||
    [];
...
result = groups.map((row) => {
    ...
    return {
        ...
        breakdown: label,
        messageCount: Number((row.count !== undefined && row.count !== null ? row.count : 0)) || 0
    };
});
