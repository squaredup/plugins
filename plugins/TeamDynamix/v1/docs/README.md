...
- **Tickets** — tickets in a ticketing application, one row per ticket, with status, priority, assignment, SLA and custom-attribute columns. Choose whether the timeframe filters on created, last-modified or closed date, or set the timeframe to **None** for the current backlog regardless of date.
...
- **The configuration item search cannot be capped at all.** Unlike tickets and assets, the CMDB search endpoint accepts no maximum-results parameter, so a very large CMDB may exceed the response size limit. If the Configuration Items tile fails to load, filter to **Active** only.
...
- **Custom fields appear as `attr.` columns.** Ticket, asset, configuration item, project and service custom attributes are flattened into columns prefixed `attr.` — for example `attr.Cost Centre`. They are hidden by default; add them from the tile's column picker.
...
- **Sandbox path is unverified.** The sandbox Web API path is assumed to be `/SBTDWebApi`. If a sandbox connection fails with a "no Web API found" error, check the correct path with TeamDynamix support.
