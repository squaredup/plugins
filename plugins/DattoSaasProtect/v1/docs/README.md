...
- **`Backup History (days)` on the Backups stream caps at 30.** The underlying `daysUntil` API parameter returns a `500 Internal Server Error` for any value above 30 — confirmed against the live API, not documented by Datto. Leave it unset for the default 10-day window.
